  ...
  const test = base.extend<HasgardFixtures, HasgardWorkerFixtures>({
    hasgard: [
      async ({}, use, workerInfo) => {
        const socketPath =
          typeof config.socketPath === "function"
            ? config.socketPath(workerInfo.workerIndex)
            : config.socketPath;
        ...
        try {
          if (config.launch) await process.start(config.launch, socketPath);
            ...
            await process.waitForSocket(socketPath, workerInfo.project.timeout);

          await rpc.connect(workerInfo.project.timeout);
          const hasgard = new HasgardApplication(rpc);
          const ping = await hasgard.ping();
          if (ping.status !== "ok")
            throw new Error(`Unexpected Hasgard ping status: ${ping.status}`);
          const primaryWindow = await hasgard.waitForWindow(
            config.windowLabel,
            fixtureTimeoutMs,
          );
          await primaryWindow
            .locator(config.readySelector)
            .waitFor({ state: "attached", timeoutMs: fixtureTimeoutMs });
          await use(hasgard);
        } finally {
          rpc.disconnect();
          await process.stop();
    ...
    window: async ({ hasgard }, use, testInfo: TestInfo) => {
      ...
      if (testInfo.status !== testInfo.expectedStatus) {
        const screenshot = await hasgard
