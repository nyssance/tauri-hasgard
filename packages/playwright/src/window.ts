...
export class HasgardApplication {
  ...
  async windows(): Promise<WindowInfo[]> {
    const value = expectRecord(
      await this.rpc.call("windows.list"),
      "windows.list result",
    );
    if (!Array.isArray(value.windows))
      throw new Error("windows.list.windows must be an array");
  ...
  async waitForWindow(
    ...
    throw new Error(`Window '${label}' did not open within ${timeoutMs}ms`);
  }
...
export class HasgardWindow {
  ...
  async snapshot(options: SnapshotOptions = {}): Promise<Snapshot> {
    const raw = expectRecord(
      await this.rpc.call(
        "snapshot",
        withWindow(this.label, options as Record<string, JsonValue>),
      ),
      "snapshot result",
    );
    if (!Array.isArray(raw.elements))
      throw new Error("snapshot.elements must be an array");
    return {
      elements: raw.elements.map((entry, index) =>
        parseSnapshotElement(entry, index),
      ),
    };
  }
  ...
  async state(): Promise<WindowState> {
    const value = expectRecord(
      await this.rpc.call("state", withWindow(this.label)),
      "state result",
    );
  ...
  async screenshot(selector?: string): Promise<Buffer> {
    ...
    const dataUrl = await this.rpc.call<string>(
      "screenshot",
      withWindow(this.label, params),
    );
    const prefix = "data:image/png;base64,";
    if (!dataUrl.startsWith(prefix))
      throw new Error("screenshot result is not a PNG data URL");
...
export class HasgardLocator {
  ...
  async click(): Promise<void> {
    await this.withTarget((target) =>
      this.window.call("click", targetParams(target)),
    );
  }

  async fill(value: string): Promise<void> {
    await this.withTarget((target) =>
      this.window.call("fill", { ...targetParams(target), value }),
    );
  }

  async type(text: string): Promise<void> {
    await this.withTarget((target) =>
      this.window.call("type", { ...targetParams(target), text }),
    );
  }

  async selectOption(value: string): Promise<void> {
    await this.withTarget((target) =>
      this.window.call("select", { ...targetParams(target), value }),
    );
  }

  async check(): Promise<void> {
    await this.withTarget((target) =>
      this.window.call("check", targetParams(target)),
    );
  }
  ...
  async textContent(): Promise<string> {
    return this.withTarget(async (target) =>
      expectString(
        await this.window.call("text", targetParams(target)),
        "text result",
      ),
    );
  }
  ...
  async inputValue(): Promise<string> {
    return this.withTarget(async (target) =>
      expectString(
        await this.window.call("value", targetParams(target)),
        "value result",
      ),
    );
  }
  ...
  async attributes(): Promise<Record<string, string>> {
    return this.withTarget(async (target) => {
      const value = expectRecord(
        await this.window.call("attrs", targetParams(target)),
        "attrs result",
      );
      return Object.fromEntries(
        Object.entries(value).map(([name, attr]) => [
          name,
          expectString(attr, `attribute ${name}`),
        ]),
      );
    });
  }
  ...
  async isVisible(): Promise<boolean> {
    return this.withTarget(async (target) => {
      const value = expectRecord(
        await this.window.call("visible", targetParams(target)),
        "visible result",
      );
  ...
  async isChecked(): Promise<boolean> {
    return this.withTarget(async (target) => {
      const value = expectRecord(
        await this.window.call("checked", targetParams(target)),
        "checked result",
      );
  ...
  async count(): Promise<number> {
    ...
    const value = expectRecord(
      await this.window.call("count", { selector: this.query.selector }),
      "count result",
    );
  ...
  async waitFor(options: WaitOptions): Promise<void> {
    ...
    throw new Error(
      `Role locator did not become ${options.state} within ${options.timeoutMs}ms`,
    );
  }
  ...
  private async resolveUnique(): Promise<HasgardTarget> {
    ...
    if (elements.length !== 1) {
      throw new Error(
        `Role locator resolved to ${elements.length} elements; expected exactly one`,
      );
    }
  ...
  private async withTarget<T>(
    operation: (target: HasgardTarget) => Promise<T>,
  ): Promise<T> {
    return this.window.runExclusive(async () =>
      operation(await this.resolveUnique()),
    );
  }
...
function parseSnapshotElement(value: unknown, index: number): SnapshotElement {
  ...
  if (entry.name !== undefined)
    element.name = expectString(entry.name, `snapshot.elements[${index}].name`);
  if (entry.value !== undefined)
    element.value = expectString(
      entry.value,
      `snapshot.elements[${index}].value`,
    );
  if (entry.checked !== undefined)
    element.checked = expectBoolean(
      entry.checked,
      `snapshot.elements[${index}].checked`,
    );
  if (entry.disabled !== undefined)
    element.disabled = expectBoolean(
      entry.disabled,
      `snapshot.elements[${index}].disabled`,
    );
  return element;
}
