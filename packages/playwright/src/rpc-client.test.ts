...
async function listen(
  respond: (request: { id: number; method: string }) => unknown | Promise<unknown>,
): Promise<void> {
  if (process.platform !== 'win32') await rm(socketPath, { force: true });
...
describe('HasgardRpcClient', () => {
  test('matches concurrent out-of-order JSON-RPC responses by id', async () => {
    ...
    const [slow, fast] = await Promise.all([
      client.call<string>('slow'),
      client.call<string>('fast'),
    ]);
  ...
  test('surfaces protocol errors with code and data', async () => {
    ...
    await expect(client.call('click')).rejects.toEqual(
      new HasgardRpcError(-32602, 'bad target', { field: 'ref' }),
    );
    client.disconnect();
  });
});
