...
//
// Run: node --test crates/tauri-plugin-hasgard/js/bridge.select.test.mjs
...
test("select by option value selects the matching option", () => {
  ...
  const hasgard = loadBridge({ queryResult: el });

  const result = hasgard.select({ selector: "select[name=role]", value: "admin" });
...
test("select by visible label selects the matching option", () => {
  ...
  const result = hasgard.select({ selector: "select[name=role]", value: "Editor" });
...
test("select throws when no option matches value or label", () => {
  ...
  assert.throws(
    () => hasgard.select({ selector: "select[name=role]", value: "zzz" }),
    /no option/i,
  );
...
test("select still rejects a non-<select> target", () => {
  ...
  const hasgard = loadBridge({ queryResult: notSelect });

  assert.throws(
    () => hasgard.select({ selector: "input[name=role]", value: "admin" }),
    /<select>/i,
  );
});
