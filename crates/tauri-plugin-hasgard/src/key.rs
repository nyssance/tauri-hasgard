...
pub fn parse_combo(combo: &str) -> Result<Combo, KeyError> {
    ...
    if trimmed == "+" {
        return Ok(Combo { modifiers: Vec::new(), key: Key::Unicode('+') });
    }
...
fn normalize_main_key(key: Key, has_modifiers: bool) -> Key {
    match key {
        Key::Unicode(ch) if has_modifiers && ch.is_ascii_uppercase() => Key::Unicode(ch.to_ascii_lowercase()),
        _ => key,
    }
}
...
fn tap_main_key(enigo: &mut Enigo, key: Key, has_modifiers: bool) -> Result<(), KeyError> {
    if let Some(keycode) = physical_digit_keycode(key, has_modifiers) {
        return enigo.raw(keycode, Direction::Click).map_err(|e| KeyError::EnigoInput(e.to_string()));
    }
    enigo.key(normalize_main_key(key, has_modifiers), Direction::Click).map_err(|e| KeyError::EnigoInput(e.to_string()))
}
...
pub fn simulate_press(combo: &str) -> Result<(), KeyError> {
    ...
    let press_outcome = (|| -> Result<(), KeyError> {
        for m in &parsed.modifiers {
            enigo.key(*m, Direction::Press).map_err(|e| KeyError::EnigoInput(e.to_string()))?;
            pressed.push(*m);
        }
    ...
    for m in pressed.iter().rev() {
        ...
        {
            release_error = Some(KeyError::EnigoInput(format!("modifier release failed (possible stuck key): {e}")));
        }
    }
...
mod tests {
    ...
    fn test_parse_empty_modifier_segment_returns_error() {
        ...
        assert!(matches!(parse_combo("Control++P"), Err(KeyError::UnknownKey(_))));
    }
    ...
    fn test_parse_unknown_modifier_returns_error() {
        assert!(matches!(parse_combo("Hyper+a"), Err(KeyError::UnknownKey(_))));
    }

    #[test]
    fn test_parse_unknown_key_returns_error() {
        assert!(matches!(parse_combo("Ctrl+NotAKey"), Err(KeyError::UnknownKey(_))));
    }
    ...
    fn test_normalize_main_key_lowercases_modified_uppercase_letter() {
        ...
        assert!(key_eq(normalize_main_key(Key::Unicode('P'), true), Key::Unicode('p')));
    }
    ...
    fn test_normalize_main_key_keeps_bare_uppercase_letter() {
        ...
        assert!(key_eq(normalize_main_key(Key::Unicode('P'), false), Key::Unicode('P')));
    }
    ...
    fn test_normalize_main_key_keeps_non_letter_keys() {
        ...
        assert!(key_eq(normalize_main_key(Key::Unicode('1'), true), Key::Unicode('1')));
        assert!(key_eq(normalize_main_key(Key::Unicode('+'), true), Key::Unicode('+')));
    ...
    fn test_normalize_main_key_keeps_non_ascii_uppercase() {
        ...
        assert!(key_eq(normalize_main_key(Key::Unicode('É'), true), Key::Unicode('É')));
    }
}
