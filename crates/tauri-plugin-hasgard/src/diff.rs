...
impl ElementKey {
    fn from(el: &SnapshotElement) -> Self {
        Self { role: el.role.clone(), name: el.name.clone(), depth: el.depth }
    }
}
...
pub fn compute_diff(old: &[SnapshotElement], new: &[SnapshotElement]) -> DiffResult {
        ...
        if let Some(new_indices) = new_groups.get(key) {
            ...
            for i in 0..pair_count {
                ...
                if !field_changes.is_empty() {
                    changed.push(ChangedEntry { old: old_el.clone(), new: new_el.clone(), changes: field_changes });
                }
            }
        }
    }

    let mut removed: Vec<SnapshotElement> =
        old.iter().enumerate().filter(|(i, _)| !matched_old[*i]).map(|(_, el)| el.clone()).collect();

    let mut added: Vec<SnapshotElement> =
        new.iter().enumerate().filter(|(i, _)| !matched_new[*i]).map(|(_, el)| el.clone()).collect();
    ...
    let sort_key = |a: &SnapshotElement, b: &SnapshotElement| {
        a.depth.cmp(&b.depth).then_with(|| a.role.cmp(&b.role)).then_with(|| a.name.cmp(&b.name))
    };
...
mod tests {
    ...
    fn test_snapshot_element_deserializes_string_value() {
        ...
        let json = r#"{"ref":"e31","role":"listitem","depth":3,"name":"single hyphen value","value":"0"}"#;
        let el: SnapshotElement = serde_json::from_str(json).expect("string value should deserialize");
