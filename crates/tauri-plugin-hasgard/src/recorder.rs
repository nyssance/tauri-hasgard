...
impl Recorder {
    pub fn new() -> Self {
        Self { state: Arc::new(Mutex::new(RecorderState { active: false, start_time: None, entries: Vec::new() })) }
    }
    ...
    pub fn status(&self) -> Value {
        ...
        let elapsed_ms: u64 = s.start_time.map_or(0, |t| u64::try_from(t.elapsed().as_millis()).unwrap_or(u64::MAX));
        serde_json::json!({
            "active": s.active,
            "count": s.entries.len(),
