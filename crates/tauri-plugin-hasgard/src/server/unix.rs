...
pub fn socket_path(identifier: &str) -> std::path::PathBuf {
    socket_dir().join(format!("tauri-hasgard-{identifier}.sock"))
}
...
async fn accept_loop(
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(e) =
                handle_connection(stream, &ctx.0, ctx.1.as_ref(), ctx.2.as_ref(), ctx.3.as_ref(), &ctx.4).await
            {
...
mod tests {
    ...
    fn unique_socket_path() -> PathBuf {
        ...
        PathBuf::from(format!("/tmp/tauri-hasgard-test-{}-{n}.sock", std::process::id()))
    }
    ...
    fn test_socket_dir_from_returns_xdg_runtime_dir_when_set_and_private() {
        ...
        let dir = std::env::temp_dir().join(format!("tauri-hasgard-xdg-test-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        std::fs::set_permissions(&dir, std::fs::Permissions::from_mode(0o700)).expect("set dir permissions");
